...
describe("schema value protocol", () => {
  it("is isolated to ./schema-value and matches schema-side decode/encode", async () => {
    expect(() => input[decodeUnknownEffect](schema)).toThrow("No protocol handler found")
  ...
  it("types decodeUnknownEffect and encodeEffect as value receivers", async () => {
    ...
    expectTypeOf(decoded).toEqualTypeOf(SchemaParser.decodeUnknownEffect(schema)(input))
    expectTypeOf(encoded).toEqualTypeOf(SchemaParser.encodeEffect(schema)(input))
  })
})
