...
import { filterOrFail, flatMap, getOrElse, map, mapBoth, mapError, match, orElse } from "./Result.ts"
...
describe("result symbol protocol", () => {
  it("chains success-channel combinators on Result branches", () => {
    ...
    expect(result[match]({ onFailure: String, onSuccess: (value) => value })).toBe("2")
  })

  it("maps and recovers failure-channel values", () => {
    const mapped = Result.fail("missing")[mapError]((error) => `error:${error}`)
  ...
  it("supports branch-wide mapping and success filtering", () => {
    ...
    expect(filtered[match]({ onFailure: (error) => error, onSuccess: String })).toBe("small:4")
  })
  ...
  it("installs methods on failure and success prototypes", () => {
    ...
    expect(Object.getOwnPropertyDescriptor(successPrototype, map)).toMatchObject({
      enumerable: false,
      writable: true,
      configurable: true
    })
    expect(Object.getOwnPropertyDescriptor(failurePrototype, map)).toMatchObject({
      enumerable: false,
      writable: true,
      configurable: true
    })
