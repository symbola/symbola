...
const sameEffect = <A, E, R>(
  before: Effect.Effect<A, E, R>,
  after: Effect.Effect<A, E, R>,
) =>
  Effect.gen(function* () {
...
const sameHashSet = <A>(
  before: HashSet.HashSet<A, A>,
  after: HashSet.HashSet<A, A>,
): void => {
...
describe("symbola slice evidence", () => {
  ...
  it.effect(
    "proves Chunk and Exit method forms against their module functions",
    () =>
      Effect.gen(function* () {
        const chunk: Chunk.Chunk<number> = Chunk.make(1, 2, 3);
        sameValue(
          Chunk.take(Chunk.append(chunk, 4), 2),
          chunk[append](4)[take](2),
        );
        ...
        sameType(
          exit[flatMap]((value) => Exit.succeed(value + 1)),
        ).toEqualTypeOf(mappedExit);

        yield* sameEffect(
          Effect.succeed(Chunk.make(1))[map]((value) => Chunk.append(value, 2)),
          Effect.map(Effect.succeed(Chunk.make(1)), (value) =>
            Chunk.append(value, 2),
          ),
        );
      }),
  );
    ...
    const entries: readonly GuardedSymbolProtocolEntry[] = [
      {
        ...
        guard: () => false,
        implementation: (() =>
          "handled") as GuardedSymbolProtocolEntry["implementation"],
      },
    ];
  ...
  it("proves HashMap and HashSet method forms against their module functions", () => {
    ...
    sameHashSet(
      hashset[union](HashSet.make(3, 4)),
      HashSet.union(hashset, HashSet.make(3, 4)),
    );
  ...
  it.effect(
    "proves Stream method forms and type gates non-Stream receivers",
    () =>
      Effect.gen(function* () {
        ...
        const flatMappedStream = stream[flatMap]((value) =>
          Stream.make(value, value * 10),
        );
  ...
  it("proves constructor/lifting method forms, laziness, and type gates", async () => {
    ...
    sameType(new Error("oops")[fail]()).toEqualTypeOf(
