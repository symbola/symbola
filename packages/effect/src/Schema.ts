...
declare global {
  interface Object {
    ...
    [required]<S extends Schema.optional<AnySchema>>(
      this: S,
    ): S["schema"]["members"][0];
    [fieldsAssign]<
...
const brandImpl = (schema: AnySchema, identifier: string) =>
  Schema.brand(identifier)(schema);
const checkImpl = (
  schema: AnySchema,
  ...checks: readonly SchemaAST.Check<unknown>[]
) =>
  schema.check(
    ...(checks as [SchemaAST.Check<unknown>, ...SchemaAST.Check<unknown>[]]),
  );
const optionalImpl = (schema: AnySchema) => Schema.optional(schema);
const requiredImpl = (schema: Schema.optional<AnySchema>) =>
