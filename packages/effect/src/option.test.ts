...
import { filter, filterMap, flatMap, flatMapNullishOr, getOrElse, map, match, orElse, orElseSome } from "./Option.ts"
...
describe("option symbol protocol", () => {
  it("chains some-channel combinators on Option branches", () => {
    ...
    expect(result[match]({ onNone: () => "none", onSome: (value) => value })).toBe("2")
  })

  it("filters and maps optional values", () => {
    const even = Option.some(4)[filter]((value) => value % 2 === 0)
    const result = even[filterMap]((value) => value > 2 ? Option.some(`large:${value}`) : Option.none())

    expect(result[getOrElse](() => "missing")).toBe("large:4")
  })

  it("supports nullable mapping and fallbacks", () => {
    const longName = Option.some("ada")[flatMapNullishOr]((name) => name.length > 10 ? name : undefined)
