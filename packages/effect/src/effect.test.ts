...
describe("effect symbol protocol", () => {
  ...
  it.effect("recovers tagged errors with catchTag", () =>
    Effect.gen(function*() {
      const recovered = lookup(999)[catchTag]("NotFound", (error) => Effect.succeed(`fallback for id ${error.id}`))
  ...
  it.effect("composes error mapping, fallback, and as", () =>
    Effect.gen(function*() {
      const mappedError = lookup(2)[mapError]((error) => error instanceof NotFound ? `missing:${error.id}` : "slow")
