...
describe("schema symbol protocol", () => {
  it.effect("mirrors SchemaParser decode effect helpers", () =>
    Effect.gen(function* () {
      const beforeUnknown = yield* Effect.exit(
        SchemaParser.decodeUnknownEffect(schema)("hello"),
      );
      const afterUnknown = yield* Effect.exit(
        schema[decodeUnknownEffect]("hello"),
      );
      const beforeTyped = yield* Effect.exit(
  ...
  it("mirrors SchemaParser decode option, result, and sync helpers", () => {
    ...
    expect(schema[decodeOption]("hello")).toEqual(
      parser.decodeOption(schema)("hello"),
    );
    expect(schema[decodeUnknownResult]("hello")).toEqual(
      SchemaParser.decodeUnknownResult(schema)("hello"),
    );
    expect(schema[decodeResult]("hello")).toEqual(
    ...
    expect(schema[decodeSync]("hello")).toBe(
  ...
  it.effect("mirrors SchemaParser encode effect helpers", () =>
    Effect.gen(function* () {
      ...
      const afterUnknown = yield* Effect.exit(
        schema[encodeUnknownEffect]("hello"),
      );
      const beforeTyped = yield* Effect.exit(
  ...
  it("mirrors SchemaParser encode option, result, and sync helpers", () => {
    ...
    expect(schema[encodeOption]("hello")).toEqual(
      parser.encodeOption(schema)("hello"),
    );
    expect(schema[encodeUnknownResult]("hello")).toEqual(
      SchemaParser.encodeUnknownResult(schema)("hello"),
    );
    expect(schema[encodeResult]("hello")).toEqual(
    ...
    expect(schema[encodeSync]("hello")).toBe(
  ...
  it("installs parser symbols as non-enumerable Object.prototype properties", () => {
    expect(
      Object.getOwnPropertyDescriptor(Object.prototype, decodeUnknownEffect),
    ).toMatchObject({
      enumerable: false,
      writable: true,
      configurable: true,
  ...
  it("types parser and combinator results as the v4 functions", () => {
    ...
    expectTypeOf(schema[decodeOption]("hello")).toEqualTypeOf(
      parser.decodeOption(schema)("hello"),
    );
    ...
    expectTypeOf(schema[encodeOption]("hello")).toEqualTypeOf(
      parser.encodeOption(schema)("hello"),
    );
    ...
    expectTypeOf(schema[brand]("Name")).toEqualTypeOf(
      schema.pipe(Schema.brand("Name")),
    );
  ...
  it("types check as the v4 schema check combinator", () => {
    ...
    expectTypeOf(schema[check](minLength)).toEqualTypeOf(
      schema.check(minLength),
    );
  });
});
