...
describe("renderArrayRuntime", () => {
  it("renders the Array protocol module without importing other domains", () => {
    ...
    expect(output).toContain("export { filter, flatMap, map } from \"../../Symbols.ts\"")
    expect(output).toContain("Effect.runSync(install(ArrayProtocol.prototype, Array.prototype))")
    expect(output).not.toContain("effect/Stream\"")
  })

  it("rejects input without supported Array runtime methods", () => {
    expect(() => renderArrayRuntime([method("effect/Array", "some", "some")])).toThrow(
      "No supported Array runtime methods were generated"
    )
  })
...
function method(effectModule: string, effectMethod: string, symbol: string): SymbolaMethod {
  return {
