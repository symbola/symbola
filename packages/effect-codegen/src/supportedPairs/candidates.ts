...
export function buildCandidate(
  ...
  if (hasCurriedReceiverSignature(exported.signatures) || syntax.returnsReceiverFunction) {
    pipe = { kind: "pipeOperator" }
...
export function buildSymbolaMethods(
  ...
  return surfaces.flatMap((surface) =>
    surface.exports
      ...
      .filter(
        (candidate) => (candidate.inferred.direct || candidate.inferred.pipe) && candidate.policy.accepted
      )
      .map((candidate) => ({
        effectModule: candidate.module,
        effectMethod: candidate.method,
        symbol: candidate.symbol,
        implementation: candidate.inferred.reasons.includes("domain-constructor")
          ? "constructor"
          : "receiverFirst",
...
function collectSyntaxHints(exported: EffectExport): {
  ...
  return {
    returnsReceiverFunction: /=>\s*\(\s*(self|schema|stream|effect|option|result|chunk|set|map)\b/.test(
      exported.snippet
    ) ||
      /function\s*\(\s*(self|schema|stream|effect|option|result|chunk|set|map)\b/.test(
        exported.snippet
      ),
    usesDual: exported.initKind === "call:dual" || /\bdual\s*\(/.test(exported.snippet)
  }
}

function hasReceiverFirstSignature(signatures: readonly SignatureInfo[]): boolean {
  return signatures.some((signature) => receiverParameterNames.has(signature.parameters[0]?.name))
}

function hasCurriedReceiverSignature(signatures: readonly SignatureInfo[]): boolean {
  return signatures.some(
    (signature) =>
      signature.returnType.includes("=>") &&
      signature.returnCallSignatures.some((inner) => receiverParameterNames.has(inner.parameters[0]?.name))
  )
}
...
function evaluatePolicy(candidate: Candidate): PolicyResult {
  ...
  if (candidate.inferred.direct && hasDomainReceiver(candidate.module, candidate.signatures)) {
    accepted.push("direct-domain-receiver")
  }
  if (candidate.inferred.direct && hasDomainConstructor(candidate.module, candidate.signatures)) {
    accepted.push("direct-domain-constructor")
  }
  if (candidate.inferred.pipe && hasDomainCurriedReceiver(candidate.module, candidate.signatures)) {
...
function hasDomainReceiver(moduleSpecifier: string, signatures: readonly SignatureInfo[]): boolean {
  return signatures.some((signature) => isDomainReceiver(moduleSpecifier, signature.parameters[0]))
}
...
function hasDomainConstructor(
  ...
  return signatures.some((signature) => isDomainConstructor(moduleSpecifier, signature))
}
...
function hasDomainCurriedReceiver(
  ...
  return signatures.some(
    (signature) =>
      signature.returnType.includes("=>") &&
      signature.returnCallSignatures.some((inner) => isDomainReceiver(moduleSpecifier, inner.parameters[0]))
  )
}
...
function isDomainConstructor(moduleSpecifier: string, signature: SignatureInfo): boolean {
