...
const symbolName = fc.constantFrom(
  "annotateLogs",
  "filter",
  "flatMap",
  "map",
...
test.prop(
  [uniqueMethods, fc.array(pairKeyArbitrary), fc.array(pairKeyArbitrary)],
  propertyOptions,
)(
  ...
  (methods, implementedKeys, generatedKeys) => {
    ...
    expect(selected).toEqual(
      methods.filter((method) => supportedKeys.has(methodKey(method))),
    );
  },
);
...
test.prop([uniqueMethods], propertyOptions)(
  ...
  (methods) => {
    expect(renderSupportedPairs(methods)).toBe(
      renderSupportedPairs([...methods].reverse()),
    );
  },
);
