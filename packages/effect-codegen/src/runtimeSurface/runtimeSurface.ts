...
export function loadImplementedSymbols(
  symbolaPackageDir: string,
): ReadonlySet<string> {
  return collectImplementedSymbols(readSymbolaSources(symbolaPackageDir));
}

export function loadImplementedPairs(
  symbolaPackageDir: string,
): ReadonlySet<string> {
...
export function filterImplementedPairs(
  ...
  return methods.filter((method) =>
    implementedPairs.has(pairKey(method.effectModule, method.symbol)),
  );
}
...
export function selectRuntimeBackedMethods(
  ...
  return filterImplementedPairs(
    methods,
    new Set([...implementedPairs, ...generatedRuntimePairs]),
  );
}

export function collectImplementedPairs(
  files: readonly SymbolaSourceFile[],
): ReadonlySet<string> {
...
function readSymbolaSources(
  symbolaPackageDir: string,
): readonly SymbolaSourceFile[] {
...
function collectImplementedSymbolsFromSource(
  text: string,
): ReadonlySet<string> {
  const symbols = new Set<string>();
