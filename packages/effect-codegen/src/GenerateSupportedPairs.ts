...
import {
  ...
  renderArrayRuntime,
} from "./generatedRuntime/Array.ts";
import {
...
const effectOxlintPackageDir = path.resolve(
  workspaceDir,
  "packages/effect-oxlint",
);
const supportedPairsPath = path.resolve(
  effectOxlintPackageDir,
  "src/internal/supportedPairs.ts",
);
const generatedArrayRuntimePath = path.resolve(
  effectPackageDir,
  "src/internal/generated/Array.ts",
);
const effectSync = Effect.sync;
const runEffectPromise = Effect.runPromise;

function formatTypeScript(filePath: string, source: string): string {
...
const command = Command.make("symbola-supported-pairs", {
  ...
  Command.withDescription(
    "Generate Symbola syntax supported pairs from Effect source",
  ),
  Command.withHandler(({ check, write }) =>
    effectSync(() => {
      ...
      const renderedPairs = formatTypeScript(
        supportedPairsPath,
        renderSupportedPairs(implementedMethods),
      );
      const renderedArrayRuntime = formatTypeScript(
        generatedArrayRuntimePath,
        renderArrayRuntime(implementedMethods),
      );
      if (write) {
      ...
      const currentArrayRuntime = fs.readFileSync(
        generatedArrayRuntimePath,
        "utf8",
      );
