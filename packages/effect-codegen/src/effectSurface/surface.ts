...
import type { EffectModuleSurface, EffectPackageInfo, JSDocTags, SignatureInfo } from "./effect.types.ts"
...
function describeMember(member: Expression): string {
  ...
  const object = memberExpression.object?.type === "Identifier"
    ? (memberExpression.object as OxcIdentifier).name
    : (memberExpression.object?.type ?? "unknown")
  const property = memberExpression.property?.type === "Identifier"
