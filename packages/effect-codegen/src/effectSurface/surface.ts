...
export function loadEffectSurface(syntaxPackageDir: string): {
  ...
  ) as EffectPackageJson;
  const sourceFiles = targetModules.map((moduleName) =>
    ...
    modules: targetModules.map((moduleName) => {
      ...
      return {
        ...
        exports: exports.map((item) => ({
          ...
          jsdoc: sourceFile
            ? collectTypeScriptMetadata(checker, sourceFile, item.localName)
                .jsdoc
            : item.jsdoc,
...
function extractLeadingTags(
  ...
  const comment = preceding.find(
    (item) => source.slice(item.end, start).trim() === "",
  );
  if (!comment) return {};
...
function collectTypeScriptMetadata(
  ...
  if (!symbol)
    return { jsdoc: collectJSDoc(declaration.jsdocNode), signatures: [] };
  ...
  return {
    ...
    signatures: type
      .getCallSignatures()
      .map((signature) =>
        formatSignature(checker, signature, declaration.name),
      ),
  };
}
...
function describeExpression(expression: Expression | null | undefined): string {
  ...
  if (expression.type === "MemberExpression")
    return `member:${describeMember(expression)}`;
