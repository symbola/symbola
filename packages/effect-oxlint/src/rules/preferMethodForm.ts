...
import { type ImportDeclarationModel, planSymbolImports, type Replacement } from "../internal/importPlanner.ts"
...
const hasTypeArguments = (call: CallExpression): boolean => call.typeArguments != null || call.typeParameters != null
...
export const preferMethodForm = {
  ...
  create(context: Context) {
    ...
    const isPipeCallee = (node: Node): node is Identifier => isIdentifier(node) && pipeNames.has(node.name)
    ...
    const importModel = (declaration: ImportDeclaration): ImportDeclarationModel => ({
      ...
      hasTypeOnlySpecifiers: declaration.importKind === "type" ||
    ...
    const nodeField = (node: unknown, field: string): unknown => isRecord(node) ? node[field] : undefined
    ...
    const pipeReplacement = (call: CallExpression): Replacement | null => {
      ...
      const target = isMethodPipeCall(call) && isMemberExpression(call.callee)
        ? call.callee.object
    ...
    const report = (
      ...
      context.report({
        ...
        ...(replacements === undefined
          ...
          : {
            fix: (fixer) =>
              replacements.map((replacement) => fixer.replaceTextRange(replacement.range, replacement.text))
          })
      })
    }
      ...
      ImportDeclaration(node: Node) {
        ...
        for (const specifier of declaration.specifiers) {
          ...
          if (declaration.source.value === "effect") {
            const imported = importName(specifier)
            const pairs = imported === null ? undefined : supportedPairsByModule.get(`effect/${imported}`)
            if (pairs !== undefined) {
              namespaces.set(specifier.local.name, pairs)
            }
            continue
