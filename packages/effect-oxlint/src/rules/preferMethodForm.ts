...
import {
  supportedPairsByModule,
  type SymbolaPair,
} from "../internal/supportedPairs.ts";
...
const isSpreadElement = (node: Node): node is SpreadElement =>
...
export const preferMethodForm = {
  meta: {
    ...
    docs: {
      description:
        "Prefer Symbola method form where a namespace call has a method equivalent.",
      recommended: false,
    },
    messages: {
  ...
  create(context: Context) {
    ...
    const hasCommentInRange = ([start, end]: [number, number]): boolean =>
      sourceCode
        .getAllComments()
        .some(
          (comment) => start <= comment.range[0] && comment.range[1] <= end,
        );
    ...
    const importModel = (
      ...
      specifiers: declaration.specifiers.flatMap((specifier) => {
        ...
        return imported === null
          ? []
          : [{ imported, local: specifier.local.name }];
      }),
    });
    ...
    const pipeReplacement = (call: CallExpression): Replacement | null => {
      ...
      const transforms = isMethodPipeCall(call)
        ? call.arguments
        ...
        for (const specifier of declaration.specifiers) {
          ...
          if (declaration.source.value === "effect") {
            ...
            const pairs =
              imported === null
                ? undefined
                : supportedPairsByModule.get(`effect/${imported}`);
            if (pairs !== undefined) {
              namespaces.set(specifier.local.name, pairs);
            }
            continue;
          ...
          if (
            declaration.source.value === "effect/Function" &&
            importName(specifier) === "pipe"
          ) {
        ...
        if (
          ...
          if (pair !== null && pair.direct) {
            report(
              direct,
              pair,
              replacementWithImports(directReplacement(direct, pair), [pair]),
            );
          }
          return;
        }
