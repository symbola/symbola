...
export function symbolaImportSource(
  pair: Pick<SymbolaPair, "module" | "symbol">,
): string {
...
export function planSymbolImports(input: {
  ...
  for (const [source, neededSymbols] of neededBySource) {
    const existingImports = input.imports.filter(
      (declaration) => declaration.source === source,
    );
    if (
      existingImports.some((declaration) => declaration.hasTypeOnlySpecifiers)
    ) {
      return blockedPlan;
    }
    ...
    const missingSymbols = [...neededSymbols].filter(
